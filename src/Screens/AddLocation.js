...
function AddLocation({ navigation,route }) {
    ...
    const Submit = async () => {
        ...
        let result = await get(`show_location`);
        console.log('result3330', result)
        if (result.success) {
    ...
    const Adddata = async (Id) => {
     ...
     let result = await post(data, 'add_location');
                            ...
                            <TouchableOpacity>
                                <View style={[{
                                 ...
                                 <View style={{flex:1,margin:17}}>
                                        <TouchableOpacity style={{ paddingLeft: 125 }} onPress={() => Adddata(item.id)}  >
