...
function ChatBox({ navigation,route }) {
    ...
    const Submit = async () => {
        ...
        setUToken(usertoken)
        console.log("route.params.ChatData",route.params.ChatData)
        let result = await get(`get-chat?sender_id=${route.params.ChatData.sender_id}&receiver_id=${route.params.ChatData.receiver_id}&page=1`);
        console.log('result3330', result.data.company_chat)
        if (result.success) {
            setMessage(result.data.company_chat)
            setAnimating(false)
        } else {
            setAnimating(false)
    ...
    const SendCompanyChat = async () => {
        ...
        var data = {
            token: UToken,
            receiver_id: route.params.ChatData.receiver_id,
            uploadedFile: singleFile != null || singleFile != undefined ? singleFile.uri : null,
            type: "company",
            text: MessageText
        }
        console.log(data, "apiData")
        let result = await post(data, 'company-chat-add');
        ...
        if (result.status) {
            ...
            // let result = await get(`get-chat?sender_id=770&receiver_id=1&page=1`);
        ...
        <SafeAreaView style={[styles.container]}>
            ...
            <View style={{ width: "100%", backgroundColor: "#1FC7B2", flexDirection: 'row', alignItems: 'center' }}>
                ...
                <View style={{ width: "55%", justifyContent: "center", }}>
                    <Text style={{ fontSize: 18, color: "white", fontWeight: "bold" }}>{route.params.ChatData.company_name}</Text>
                </View>
            ...
            <View style={{ backgroundColor: 'white', paddingVertical: '2%' }}>
                {singleFile != null ?
                    <View style={{ height: 150, width: '100%', borderRadius: 30, alignItems: "center", alignSelf: "center", backgroundColor: 'lightgrey' }}>
