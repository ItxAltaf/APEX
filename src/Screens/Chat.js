...
import { View, StyleSheet, Button, ActivityIndicator, Text, StatusBar, TouchableWithoutFeedback, TouchableOpacity, Image, ScrollView, SafeAreaView, FlatList, Platform } from 'react-native';
...
function Chat({ navigation }) {
    ...
    const Submit = async () => {
        ...
        let result = await get(`get-recent-chat-list?token=${usertoken}`);
        console.log('result3330', result.data)
        if (result.success) {
            setMessage(result.data.recent_company)
            setAnimating(false)
        } else {
            setAnimating(false)
                        ...
                        <View style={[{
                            ...
                            <View style={{ flexDirection: 'row', width: "100%", alignItems: "center" }}>
                                ...
                                <View style={{ width: "70%", paddingHorizontal: 10 }}>
                                    ...
                                    <TouchableOpacity style={styles.button} onPress={() => navigation.navigate('AdminChatBox')}>
                    ...
                    <View style={{ width: "100%" }}>
                        {
                            message.map((item, index) => {
                                return (
                                    <TouchableOpacity onPress={()=>navigation.navigate('ChatBox',{ChatData:item})} key={index}>
                                        <View style={{ width: "90%", flexDirection: "row", height: 100, alignItems: "center", alignSelf: "center", justifyContent: "space-between" }}>
                                            <View style={{ flexDirection: "row", alignItems: "center", }}>
                                                <View style={{ height: 60, width: 60, borderRadius: 30, alignItems: "center", justifyContent: "center", marginRight: 10 }}>
                                                    <Image source={item.picture == null || item.picture == undefined ? profile_icon : { uri: item.picture }} style={{ width: 60, height: 60, resizeMode: "contain" }} />
                                                </View>
                                                <View style={{}}>
                                                    <Text style={{ fontSize: 16, fontWeight: "bold" }}>{item.company_name}</Text>
                                                    <Text style={{ fontSize: 14, color: "#797A79" }}>{item.text}</Text>
